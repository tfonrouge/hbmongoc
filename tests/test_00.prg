...
PROCEDURE main()
    ...
    mongoc_collection_drop( collection )
        ...
        ? hb_tton(hb_dateTime()), hb_dtToUnix(hb_dateTime())
        BSON_APPEND_DATE_TIME( bson, "dtToUnix", hb_dtToUnix(hb_dateTime()) )
        hb_bson_append( bson, "integer", 2^32 + 1 )
        BSON_APPEND_INT32( bson, "integer32", 2^32 + 1 )
        hb_bson_append( bson, "long", hb_dtToUnix(hb_dateTime()) )
        BSON_APPEND_INT64( bson, "long64", hb_dtToUnix(hb_dateTime()) )
